...
export default function AuthStep({ user, onChange, onNext, onBack }) {
  ...
  const switchMode = (m) => { setMode(m); setError(null); setEmailSent(false) }
  ...
  // ── Sign Up — creates account + sends confirmation email ──────────────────
  ...
  // ── Log In — sends magic link to existing account ─────────────────────────
  const handleLogin = async () => {
    ...
    setLoading(false)
    if (authError) {
      // If no account found, nudge them to sign up
      if (authError.message.toLowerCase().includes('not found') || authError.status === 422) {
  ...
  const isSignUpValid = name.trim().length >= 1 && email.trim().includes('@')
  const isLoginValid  = email.trim().includes('@')
    ...
    return (
      <div className="w-full max-w-lg step-enter flex flex-col items-center text-center">
        ...
        <p className="text-slate-400 mb-1">
          {mode === 'signup' ? 'We sent a confirmation link to' : 'We sent a magic link to'}
        </p>
        <p className="font-bold text-gather-600 text-lg mb-5">{email}</p>
        <p className="text-sm text-slate-400 mb-8 max-w-sm">
          {mode === 'signup'
            ? 'Click the link in your email to confirm your account — then come back and continue.'
            : 'Click the link in your email to sign in — then come back and continue.'
          }
        </p>
        <button
          onClick={onNext}
    ...
    <div className="w-full max-w-lg step-enter">
      ...
      {mode === 'signup' && (
        <div className="space-y-4 animate-fade-in">
          ...
          <p className="text-xs text-slate-400">
            We'll send a confirmation link to your email — no password needed.
          </p>
          {error && (
      ...
      {mode === 'login' && (
        <div className="space-y-4 animate-fade-in">
          ...
          <p className="text-xs text-slate-400">
            We'll send a magic link to your inbox — click it to sign in instantly.
          </p>
          {error && (
